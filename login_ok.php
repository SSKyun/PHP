...
if($row = $stmt->fetch(PDO::FETCH_BOTH)) {
    if($row['pwd'] != $pwd) {
        ?>
        <script>alert('비밀번호가 일치하지 않습니다.');location.href='<?=$_user_options["loginPage"]?>';</script>
        <?php
        return;
    }
} else {
    ?>
    <script>alert('입력된 정보와 일치하는 정보가 없습니다.');location.href='<?=$_user_options["loginPage"]?>';</script>
    <?php
    return;
}
//로그인 성공

...
<html>
  ...
  <body>
    ...
    <div class="w-100 px-3">
      <div class="col-3 offset-9">
        ...
        <button class="btn btn-secondary" type="button" onclick="golist();">전체보기</button>
          ...
          <input id="query" type="text" class="form-control" value="<?=$q?>" placeholder="검색어 입력">
    ...
    <script>
      ...
      function search(){
        let options = []; 
        options.push({type: 'text',selector : '#query',regex:/^.{0,1}$/,errorMsg:'검색어를 2개이상 입력해주세요.'});
        let result = kyunCheckForm(options);
        let q = document.querySelector('#query').value;
        q = encodeURIComponent(q);//인코드에서 사용한 문자가 유니코드로 바뀌어서 넘어간다.
        location.href = `?q=${q}`;
      }
    </script>
  </body>
